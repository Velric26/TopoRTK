...
debugmode::Session session;debugmode::Log logbook;
}
...
void debug_enable_local(bool enabled){
  ...
  else {session.disable();logbook.clear(now);}portEXIT_CRITICAL(&guard);
}
...
bool debug_status(char *out,size_t capacity){
  char peer[80];peer_update_label(peer,sizeof(peer));
  const bool enabled=session.active(millis());
  const int n=std::snprintf(out,capacity,"{\"version\":1,\"firmware\":\"%s\",\"boot_id\":%lu,\"uptime_ms\":%lu,\"enabled\":%s,\"passive\":true,\"ota_available\":true,\"peer_update_notice_available\":true,\"peer_status\":\"%s\",\"update_paused\":%s,\"update_locked\":%s}",kWebUiVersion,static_cast<unsigned long>(web_boot_id()),static_cast<unsigned long>(millis()),enabled?"true":"false",peer,ota_paused()?"true":"false",ota_locked()?"true":"false");
