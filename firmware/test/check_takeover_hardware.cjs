...
try{for(const ip of ['192.168.100.20','192.168.100.19']){
 ...
 try{
  ...
  async function take(page){const old=await page.evaluate(()=>sessionStorage.getItem('topoControlToken'));await page.locator('#pairPanel').evaluate(x=>x.open=true);await page.locator('#takeControl').click();await page.waitForFunction(old=>sessionStorage.getItem('topoControlToken')!==old&&document.querySelector('#controlState').textContent==='You control this instrument',old);latest=await page.evaluate(()=>sessionStorage.getItem('topoControlToken'));assert.equal(latest.length,32);return latest}
