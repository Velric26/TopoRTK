...
(async()=>{fs.mkdirSync(out,{recursive:true});const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 await page.route('**/*',async route=>{const r=route.request(),url=new URL(r.url());
  ...
  if(r.method()==='POST'){const d=r.postDataJSON();posts.push({path:url.pathname,data:d});
   ...
   if(d.op==='corrections')state={...state,corrections:{transport:d.transport,session:d.transport==='sik'?(d.session||123456789):0,station:-1,queue_expired:0,wire_errors:0,output:{forwarded:0}}};
   ...
   if(d.op==='pairtest')state={...state,...d,kind:'paired_rtcm_faults',state:'armed',busy:true,remaining_seconds:0,integrity_violations:0,pair_pass:false};
 ...
 await page.locator('#confirm').check();await page.locator('#run').fill('913226');await page.locator('#pairarm').click();await page.waitForFunction(()=>state?.kind==='paired_rtcm_faults'&&state.busy);assert(await page.locator('#pairarm').isDisabled());assert(await page.locator('#selftest').isDisabled());
  assert.deepEqual(posts.at(-1).data,{op:'pairtest',run:913226,seconds:60,profile:'clean',confirm:true});assert(await page.locator('#profile').isDisabled());
  state={...state,state:'done',busy:false,pair_pass:true,local_pass:true,peer_report_received:true,uart:{observed:true,fifo_overflow:0,buffer_full:0,frame_errors:2,breaks:0,rx_backlog_peak:180,max_service_gap_ms:30}};await page.waitForFunction(()=>document.querySelector('#result').textContent.includes('eligible RTCM'));assert.equal((await download()).kind,'paired_rtcm_faults');assert.match(await page.locator('#reportDetails').innerText(),/No faults are deliberately injected/);assert.match(await page.locator('#uartResult').innerText(),/2 framing errors/);
  await page.locator('#run').fill('913227');await page.locator('#profile').selectOption('injected');await page.locator('#pairarm').click();await page.waitForFunction(()=>state?.run===913227&&state.busy);assert.equal(posts.at(-1).data.profile,'injected');await page.locator('#cancel').click();await page.waitForFunction(()=>!state.busy);assert.match(await page.locator('#reportDetails').innerText(),/deliberately injected/);
 await page.locator('#probe').click();await page.waitForFunction(()=>document.querySelector('#result').textContent.startsWith('Checking the local'));assert(await page.locator('#cancel').isDisabled());assert.deepEqual(errors,[]);
 state={...state,state:'idle',busy:false,run:0};await page.waitForFunction(()=>!document.querySelector('#liveStart').disabled);
 await page.locator('#liveStart').click();await page.waitForFunction(()=>document.querySelector('#routeState').textContent.includes('123456789'));
 assert.deepEqual(posts.at(-1).data,{op:'corrections',transport:'sik',session:0,confirm:true});
 for(const id of ['arm','probe','pairarm','selftest'])assert(await page.locator('#'+id).isDisabled());
 assert(await page.locator('#sessionLabel').isHidden());
 offline=true;await page.waitForFunction(()=>document.querySelector('#connection').textContent.startsWith('Disconnected'));assert(await page.locator('#liveWifi').isDisabled());offline=false;
 ...
 state={...state,role:'ROVER'};await page.waitForFunction(()=>!document.querySelector('#sessionLabel').hidden);
 await page.locator('#liveSession').fill('123');await page.locator('#liveStart').click();assert.match(await page.locator('#message').innerText(),/Copy the current session/);
 await page.locator('#liveSession').fill('123456789');await page.locator('#liveStart').click();await page.waitForFunction(()=>document.querySelector('#routeState').textContent.includes('123456789'));
 assert.equal(posts.at(-1).data.session,123456789);assert.deepEqual(errors,[]);
