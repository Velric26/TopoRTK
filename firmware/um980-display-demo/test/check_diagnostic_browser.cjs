...
(async()=>{fs.mkdirSync(out,{recursive:true});const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 assert.deepEqual(posts.at(-1).data,{op:'pairtest',run:913226,seconds:60,confirm:true});
  state={...state,state:'done',busy:false,pair_pass:true,local_pass:true,peer_report_received:true};await page.waitForFunction(()=>document.querySelector('#result').textContent.includes('eligible RTCM'));assert.equal((await download()).kind,'paired_rtcm_faults');
