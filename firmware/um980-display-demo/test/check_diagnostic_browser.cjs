...
const root=path.resolve(__dirname,'../../..'),out=path.join(root,'tests/2026-09-12-standalone-tablet');
...
(async()=>{fs.mkdirSync(out,{recursive:true});const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 fs.writeFileSync(path.join(out,'browser-results.json'),JSON.stringify({result:'PASS',checks:['live connection and role','Both roles use takeover without a PIN','explicit claim/release and reload retention','confirm/arm/state lock','offline and stale controls disabled','current failed report never replaced with old pass','report download','320/390/768/1280 widths and 200% text','probe has no ineffective cancel button','no page errors']},null,2));console.log('PASS: diagnostic browser lifecycle, reports, offline state and responsive layout');
