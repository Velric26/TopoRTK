...
3. Add two warned buttons and countdown/results using specified API/profile. *(R9b — not started; the Settings page currently renders a test's phases and result read-only, with kind-aware copy.)* Verify warning cancellation sends no command, disconnect does not stop admitted tests, missing peer results never pass, finite tests end autonomously and restoration is visible.
