...
Deviations from step 4's prescribed structures, recorded rather than smoothed over: the queue is **8 records, not 16**, and there is **no dedicated low-priority writer task** — the commits are bounded inline in the service turn. Both changes are smaller than prescribed and cheap to justify (a turn's producers emit at most 6 rows today, and the bounded inline commit already satisfies "no blocking of corrections" in the host failure case), but per step 5 the scheduling change is **not accepted** until a measured saturated-link/logging run on real hardware shows no new UART overruns, bounded backlog and no correction-age, touch/HTTP or survey-durability regression. If that run fails the gate, the plan's instruction applies: retain synchronous behavior and mark R10b not accepted rather than masking overload.
