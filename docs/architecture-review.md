...
**R10 step 2 status (2026-09-15): partially met, still open.** The extraction itself is complete and the suites exercise the production sources, but several harnesses still build them by *regex-stripping includes into a generated translation unit* rather than compiling production files against a test adapter: `test/run_host_tests.py` writes `.pio/host_*.cpp` copies for the UI, transport, presenter and status modules and one generated TU for the firmware, and the update/OTA runner does the same for its modules. The doubles themselves are adapter-style (`test/host_hardware.h`, `host_hardware.cpp`, `ota_hardware.h`, `peer_hardware.h`), so the remaining work is the copy mechanism, not the doubles. This is not cosmetic: the regex path already hid one real defect (a moved panel include left `RGB565_RED` undefined in `main.cpp` for the PlatformIO build while the host copy still defined it), which is exactly the class of failure the step exists to remove.
