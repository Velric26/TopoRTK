...
- `survey_ui.h` and `survey_tools_ui.h` contain **28,219 and 29,715 bytes** of densely packed embedded HTML/JavaScript. The latter repeatedly replaces global `render`, `controls`, `switchTab` and collection handlers (16–18, 43–45, 52–54, 72–74, 79, 92). `ota_ui.h`, line 16, similarly wraps Debug controls. *Superseded 2026-09-15: those headers are deleted, the sources are the canonical `firmware/web/` files, and `api.js`/`navigation.js` replace the per-page token plumbing and the injected Debug tab.*
...
- (Resolved 2026-09-15, see R10 step 2 below) [test/run_host_tests.py](../firmware/test/run_host_tests.py) concatenated source files and regex-stripped hardware headers at the time of this review; browser tests extract C++ raw strings. Moving code without moving these verification boundaries would preserve fragile tests.
...
| R10c | P2; after R10a (operator request 2026-09-15) | **Designed software restart**: command-triggered `esp_restart()` (the power-cycle equivalent) on the instrument's own surfaces, gated like every other disruptive action, observable before the reset and documented with the post-flash path | 3/5 | **Sol**: reuses the diagnostics admission, the two-tap confirm and the published snapshot. **Astra** reviews the gates and the grace window. |
| R10d | P2; operator request 2026-09-15; needs a switched radio supply (hardware) | Power-on default is Wi-Fi, and the SiK radios stay unpowered (or in a confirmed low-power mode) until radio communication is requested | 3/5 | **Astra**: supply switching, inrush and UART back-power rules, and the interaction with pair negotiation; **Sol** implements the firmware gate once the hardware exists. |
...
**Remediation (2026-09-15):** F01–F04, F06, F07 and the acceptance-discovered F08 are repaired with regression cases and an offline integration set that passes; the review document's status table records what is fixed and the [remediation record](../tests/2026-09-15-r06r/README.md) carries the evidence and the on-hold items. F05 (removing the manual advanced-diagnostic matching) is fixed as well: one instrument requests a paired test whose shape (profile, duration, rate, direction) rides the operation wire to both peers, the diagnostics page and the bench drivers no longer use matched six-digit codes, and a shape the tested medium does not offer is refused before admission — see the [F05 record](../tests/2026-09-15-f05/README.md).
...
**R10a status (2026-09-15):** the extraction is complete to the plan's boundary — `main.cpp` went from 2128 to **630 lines** and holds service wiring, the `setup()`/`loop()` ordering, the gesture dispatch and the one `instrument_status::Inputs` composition each surface consumes. The seven new owners are `instrument_status`, `gnss_service` (sole UART1), `correction_service` (COM2 output path), `device_settings`, `diagnostic_log`, `board_hardware`, `usb_console`, `correction_wifi` (Wi-Fi envelope policy), `ui_presenter` (snapshot → `UiFrame`) and `status_surface` (web-status JSON), with the survey bridge in `survey_service`. Every slice was proven by comparing the host suite's status snapshots and all UI previews byte-for-byte against the pre-change commit: the only delta across all six slices is the suite's own simulated clock (`uptime_ms`), which the added cases advance further.

**Deviation from the size target, recorded rather than smoothed over:** the plan names "approximately 100–200 lines". The root holds 630, and the remainder is not a relocated monolith — it is service wiring, the loop order, `is_base()`, the one `instrument_status::Inputs` composition each surface consumes, the gesture dispatch and the render entry points the host suite calls. Two contracts the plan itself sets keep those in the root: slice 1's input struct is explicitly the composition root's mapping, and slice 4's settings hooks exist so the modules never reach the NVS/UART themselves. Shrinking further would mean moving presentation *policy* (gesture→action) and cross-service state assembly into owners the plan does not name, so it is left as a deliberate, documented choice rather than claimed as done.

**R10b (2026-09-15):** measured bounds rather than guesses. `service_input` drained whatever the ring held (one second of Base COM2 output = 1492 bytes, 129.5 ms of line time, in a single turn); it now reads an unconditional **512-byte budget** (44 ms, ~17× an ordinary turn's ~30 bytes) and past that budget keeps reading only while the backlog exceeds a **256-byte margin**, so the effective per-turn cap is still the ring's worth — the plan's 2,048-byte cap is honoured as the ceiling while the ordinary budget keeps loop latency bounded — and receiver bytes are never discarded to save time. The optional CSV writers could cost a churn turn 160 ms against a 40 ms card and 80 ms inside the receiver's own observer call; they now queue into a bounded 8-row ring and commit at most one solution slot plus two event rows per turn, so the per-turn card cost is independent of the turn's churn and a stalled or failing card skips rows with every drop counted. Journal semantics untouched. **Still owed on hardware:** the plan asks for a measured saturated-link/logging run (step 5) — no new UART overruns, bounded backlog, no correction-age/touch/HTTP/durability regression — which needs the bench.


#### R10d — Radios unpowered until radio communication is requested (operator request, 2026-09-15)

The request: **the default communication mode at power-on is Wi-Fi, and the SiK radios should be off — or in a low-power mode — until the user switches to radio communication.** Both halves are recorded here; the second one is not implemented and cannot be until the hardware exists.

**Status of the first half: already true.** A fresh instrument defaults to Wi-Fi (the checked record's absence keeps the historical Wi-Fi default in `link_service::load_records`, and a *stored* selection is restored instead — an instrument that was last used on Radio comes back on Radio, which is the intended behaviour). What is new is that the radios themselves keep drawing whatever an idle SiK module draws even when Wi-Fi carries production: nothing in the current design controls their supply.

**Hardware prerequisite (open).** The 1 W SiK module is fed directly from the 3S pack, so "off" needs a switched rail: a high-side load switch or P-FET on the radio's supply, driven by a free GPIO, with inrush handling and — importantly — a rule for the UART lines, because the ESP32's TX would otherwise back-power an unpowered module through its own input protection. Series resistance or a level-safe arrangement on TX/RX is part of the electrical task, as is choosing the GPIO and recording it in the hardware documents. If the specific SiK firmware on these modules turns out to have a dependable low-power command, that is an acceptable substitute — but it must be established on the actual module rather than assumed.

**Firmware contract once the switch exists.**
1. Power-on leaves the radio unpowered while the stored selection is Wi-Fi, and energizes it only when radio communication is requested: a radio probe, a radio test, or a pair-wide cutover whose target is Radio (energize before staging, not at commit).
2. Energizing includes the module's own boot and settling time inside the existing windows — the 20-second negotiation cap and the probe's current 1.2-second pre-command wait are the places to account for it — and the switch must never toggle while a radio frame is being transmitted.
3. A dead UART because the module is intentionally off must not read as a fault: status says the radio is off (Wi-Fi selected) rather than "no response", the probe powers the rail first, and no radio fault is latched.
4. Nothing about the pair, session, replay or correction gates changes: this is a supply decision, not a protocol one.

**Acceptance (after the hardware change).** Idle current at the pack is measured with the radio off and compared with the present idle draw; a Radio selection from Settings (and the touchscreen Link-mode page) brings the module up and pairs without operator intervention; a refused or cancelled request leaves the rail off again; a probe on an unpowered radio powers it first and reports the identity; no false radio-fault status appears; and the G01/R11 rows are re-run with the switch in both states.

#### R10c — Designed software restart (operator request, 2026-09-15)

A full SoC reset (`esp_restart()`) is the software equivalent of the power cycle an operator would otherwise have to perform, and the escape hatch for a state that cannot be recovered in place. This session needed exactly that after a pre-fix build wedged a unit, and the G01 restart rows used to assume physical power cycles for every restart. The operator asks for the capability to be documented as a feature, available on command while Debug is on, and usable after a flash.

**Contract**

1. **One admission path, the instrument's own surfaces.** `POST /api/v1/diagnostic` with `{"op":"restart","confirm":true}`, the Debug page on the touchscreen with the same two-tap confirmation the other disruptive controls use, and a web Debug card (this item's remaining work). No new privilege: the request rides the diagnostics queue.
2. **Gates.** Same-origin check and controller lease as every diagnostic action, an explicit `confirm`, refusal while an update is transferring (that path owns the reboot), refusal while a run or probe is in flight, and the survey/diagnostic reservation as the proof that nothing is collecting or writing. The reservation is then *held* through a 500 ms grace window so nothing new starts under a chip that is about to reset.
3. **Observable before the reset.** The caller's 202, the published `restart_pending` and the serial line all leave the instrument before `esp_restart()` runs on a later service turn, so an operator sees that the request was admitted even though the connection dies with the reset.
4. **After a flash.** OTA already reboots as part of its own flow, and USB flashing resets the chip when the tool finishes (`--after hard_reset`); this command exists for the case where a clean boot is wanted *without* flashing, and it stays available in Debug mode, where an operator is looking during bring-up. It is deliberately not a substitute for flashing: nothing is persisted by the restart.
5. **Recovery semantics.** Because the reservation is held, the instrument refuses new disruptive work until it reboots, and the reboot clears RAM state — a wedge, a held reservation, a `recovery_required` flag — exactly as a power cycle would.
6. **Acceptance.** `run_pair_matrix.py` restarts each unit in turn through this operand and requires new boot ids with the pair renegotiating by itself; the host suite pins the touchscreen confirmation, the single-request shape, the refusal copy and the armed-tap expiry; the web card gets the same treatment in the Debug browser check.

**Status (2026-09-15): implemented on all three surfaces.** The API op, the touchscreen Debug control and the web Debug card ship together with the gates, the grace window, the published `restart_pending` and the refusal text, and each surface carries a case that fails without it: the host suite pins the two-tap confirmation, the single-request shape, the refusal copy and the armed-tap expiry; the Debug browser check pins the card's gating (controller lease plus confirmation), that a refused request reports the diagnostics layer's own text rather than the queue's generic status, and that an accepted one is confirmed by reading the published state instead of trusting the 2xx. The pair-matrix restart phase runs at the bench.

**R10 step 2 status (2026-09-15): complete.** Every harness now compiles the production translation units themselves. Each harness carries an adapter directory (`test/host_adapter`, `test/ota_adapter`, `test/peer_adapter`) holding one header per platform name a production source includes — `Arduino.h`, `WiFi.h`, `SD_MMC.h`, `Arduino_GFX_Library.h`, `Preferences.h`, `esp_ota_ops.h`, `mbedtls/sha256.h` and the rest — each forwarding to that harness's existing double, with the runner passing `-I` to it. No source text is copied, stripped or rewritten, and the composition TU is now a list of `#include`s of the real files rather than pasted code, so what the cases execute is the source under review. Completeness is enforced by the compiler rather than by a regex: a source that moves or renames a platform include fails the harness build by name. That is exactly the defect class that previously hid a missing panel header from the host build (the old copy still defined `RGB565_RED` while PlatformIO did not), and the mechanism was falsified deliberately — planting a renamed include in `src/ui_screens.cpp` fails the host build with `fatal error: Arduino_GFX_Library_v2.h: No such file or directory`, and restoring it passes. The eight suites and byte parity across all 60 rendered artifacts are unchanged from the pre-change baseline.

Two exceptions are deliberate and documented in `test/README.md`: the composition root's sources must still be one translation unit because the cases call its internal entry points directly (declaring those boundaries is a separate, larger change), and `IPAddress.h` is left unadapted on purpose so that the `__has_include` branches the host build compiled before are the ones it still compiles. ArduinoJson remains a real library dependency, not an adapter. The peer harness keeps its invariant guard: `src/peer_update.cpp` is included inside a namespace twice, so the runner asserts that the source's include set is still covered by the prelude — a read-only check that names what to add, not a rewrite.


**R10b status (2026-09-15): implemented, acceptance pending.** Measured bounds rather than guessed ones. `service_input` drained whatever the ring held (one second of Base COM2 output = 1492 bytes = 129.5 ms of line time in one turn); it now reads an unconditional **512-byte budget** (44 ms, ~17× the ~30 bytes an ordinary turn receives) and past that budget keeps reading only while the backlog exceeds a **256-byte margin**, so a burst is still drained inside the turn and the effective ceiling remains the ring's worth — the plan's 2,048-byte cap is honoured as that ceiling. Receiver bytes are never discarded to save time. The optional CSV writers could cost a churn turn 160 ms against a 40 ms card, and 80 ms inside the receiver's own observer call; they now queue and commit at most one solution slot plus two event rows per turn, so the per-turn card cost no longer follows the turn's churn, stalls and failures skip rows with every drop counted, and the receiver's observer call performs no card opens.

Step 4's structures are now in place: a fixed **16-record queue**, each record carrying its own **160-byte destination and 512-byte text** copied in at enqueue time (so a session change cannot misroute a queued row), event lines held to 256 bytes and solution lines to 320 with truncation detected through the formatter's return, **oversized and full-queue drops counted and published**, and a card that accepts the open but refuses the bytes marks optional logging unavailable and stops touching the card until a bounded retry window elapses. The per-second sample keeps its own record, so a deep backlog cannot delay it. Static footprint grows to ~11.4 KiB, compile-enforced.

One deviation remains, by reasoned choice: the writer is a **dedicated bounded drain stage on the loop** (called last in the turn, at most three records per call, a zero-timeout take of `survey_sd_lock`) rather than a separate low-priority FreeRTOS task. The argument is that a second task could only poll the same SD mutex from another context — SD-bus contention and queue-index races with the journal's worker — or block on the card, which is exactly what must not happen; the plan's real requirement is a bound on the turn, and the budgeted stage gives that deterministically both on the device and in the offline cases. Per step 5 the scheduling change is still **not accepted** until a measured saturated-link/logging run on real hardware shows no new UART overruns, bounded backlog and no correction-age, touch/HTTP or survey-durability regression. If that run fails the gate, the plan's instruction applies: retain synchronous behavior and mark R10b not accepted rather than masking overload.
...
**R11 (2026-09-15, staged):** the scripted gate and its runbook are written in the [R11 gate record](../tests/2026-09-15-r11/README.md) with the release candidates `0.11.33-arch-r10` built and hashed; `run_pair_matrix.py` covers identity/roles, boot convergence, local selection, four cutovers, a test per medium, cancellation, restarts of each unit through the designed restart, the documented refusals and an optional guarded OTA with saved-state checks. The isolated-medium matrix, the planned OTA, the sunlight check, the touchscreen verification and the outdoor SiK RTK/recovery row remain operator work, and the pair currently needs one physical action to take the new build.
...
| [pair_session.cpp](../firmware/src/pair_session.cpp), [link_operation.cpp](../firmware/src/link_operation.cpp) | `Engine::receive/next/committed` and `settle`: discovery may never replace a proven identity, a completed challenge is retired, and every terminal outcome (including `recovery_required`) must release the staging reservation while only the durable pending record stays authoritative. |
| [link_service.cpp](../firmware/src/link_service.cpp) | `load_records`, `persist_confirmed`, `route_control`: absence is not corruption, validation accumulates, and a control envelope acts only on the medium it physically arrived on. |
| [gnss_service.cpp](../firmware/src/gnss_service.cpp) | `service_input`, `service_startup`, `service_profile`: the sole UART1 owner; per-turn read budget with the backlog margin, and profile acknowledgement before `profile_applied`. |
| [correction_service.cpp](../firmware/src/correction_service.cpp) | `admit`, `service_output`: the only path to COM2, with the selected-link admission gates and the station guard. |
| [diagnostic_log.cpp](../firmware/src/diagnostic_log.cpp) | the optional CSV writer: bounded queue with its own destination per record, counted drops, and a failure that disables optional logging instead of blocking corrections. |
| [correction_wifi.cpp](../firmware/src/correction_wifi.cpp) | `handle_rtcm_packet`, `send_rtcm_packet`, `service`: Wi-Fi correction identity/replay admission; unselected-medium copies are never fed. |
| [ui_presenter.cpp](../firmware/src/ui_presenter.cpp), [ui_screens.cpp](../firmware/src/ui_screens.cpp) | the frame builder and the renderers: a page must clear everything it draws (a button's subtitle sits at `rect.y+46`, so its rect must contain it) and the two-tap controls share one confirmation state. |
| [link_diagnostic.cpp](../firmware/src/link_diagnostic.cpp) | `diagnostic_request`, `radio_owner`, `maintain_radio_ownership`: the local actions (selection, probe, self-test, restart) and the bounded UART2 ownership handoff that a terminal test must release. |
