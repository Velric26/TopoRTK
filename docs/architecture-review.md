...
**R10a (in progress, 2026-09-15):** the status/readiness policy moved into `instrument_status.cpp` behind an explicit input contract and the UM980 receiver service moved into `gnss_service.cpp` as the sole UART1 owner (published snapshot, typed profile/command requests, unchanged allowlist and acknowledgement-before-verified semantics). Both moves are behaviour-identical by byte comparison of the host suite's status snapshots and all 28 UI previews against the pre-change commit, with the eight offline suites and seven browser checks passing and both PlatformIO environments building. Remaining slices: the correction service, the settings/storage split and the Wi-Fi framing region, then a composition-only `main.cpp`.
