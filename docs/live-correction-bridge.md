...
**Normal selection is pair-wide and lives in Settings.** Open **Settings** (`/settings`) on either role, choose **Radio** or **Wi-Fi** and confirm; one request switches the pair, coordinated by the Rover unit and admitted once at a time. The page keeps the durable selected medium, the pending candidate with its live operation (state, reason, coordinator, phase countdown) and the pair's connectivity visibly separate, renders a queued request as queued rather than as success, and reads the outcome back from the same endpoint. The per-instrument local control on `/diagnostics` described below is the **recovery** path for a pair that cannot confirm itself, not the everyday way to switch.

1. Nothing is copied between instruments. The logical Base mints a fresh session during current-boot negotiation on the selected medium and the Rover follows automatically; both then report **connected** with the same internal session. Negotiation is capped at 20 seconds, and an unreachable peer keeps reporting its own state instead of a success.
2. Both instruments must select the same medium. A mismatch simply never pairs: each instrument reports its own state and keeps production output inhibited. Selecting one medium never forwards the other medium's corrections.
3. Before a CRC-valid 1005/1006 base reference arrives, they show **Waiting for a base reference**. With the current receiver profile the reference repeats every ten seconds. The Rover drops observations until it has a reference for the selected station.
...
**Pair-wide selection API:** `GET /api/v1/settings` reports the durable selection revision, the selected and candidate medium, peer connectivity and the current operation; `POST /api/v1/settings` with `{"id":"<32 hex>","revision":N,"op":"link.select","transport":"sik"|"wifi","confirm":true}` asks the **pair** to switch, coordinated by the Rover unit and admitted once at a time with the same survey/receiver/OTA exclusions. The request returns 202 and its outcome (including restoration of the previous medium after a failed cutover) is published in the same endpoint, so a queued request is never reported as "connected". The Settings page consumes this API and is the normal operator path on both roles.

**Local recovery selection:** if the pair cannot confirm itself, the per-instrument form is **Radio** or **Wi-Fi** under **Live correction routing** on `/diagnostics` (the API form is `{"op":"corrections","transport":"sik"|"wifi","confirm":true}`; there is no session field). It selects only the instrument you are connected to, so a pair must be set on each instrument; the choice is stored on that instrument and restored at boot, and Wi-Fi remains the default until a medium is selected. This is a deliberate recovery step, not normal pair coordination.
