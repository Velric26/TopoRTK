...
1. Open `/diagnostics` on Base and take control. Under **Live correction routing**, choose **Start new SiK session**.
2. Copy the displayed session number to Rover's `/diagnostics`, take control, then choose **Join Base SiK session**. This number identifies the correction session; it is not a password or control PIN.
3. Verify both show the same session. Before a CRC-valid 1005/1006 base reference arrives, they show **Waiting for a base reference**. With the current receiver profile the reference repeats every ten seconds. The Rover drops observations until it has a reference for the selected station.
4. Use **Use Wi-Fi corrections** on each instrument to return to the prior Wi-Fi path and enable diagnostics. Starting a live session preserves saved radio/self-test reports and job records. Diagnostic tests/probes are rejected while SiK is selected.

Requests are asynchronous. Check the displayed route/error after submitting. The survey admission gate can reject a request during an occupation, receiver operation, queued job write or brief service contention; finish the operation or retry when idle. A retried Rover join to its already-selected session is a no-op and preserves replay history. A new Base start intentionally generates a new session, requiring Rover to join again.

**Restart behavior:** this preview deliberately does not persist a live session or sequence history. Wi-Fi is the startup default. After either instrument restarts, start a fresh Base session and join it from Rover. Do not reuse an old session number. Persistent paired identity and automatic session negotiation are later work. Role changes also end the live selection. Radio/output faults inhibit forwarding rather than silently switching an active session to Wi-Fi; a new selection clears the fault. Receiver outages retain the selected radio session and recover with new valid messages, a base reference and receiver-confirmed quality.

The SiK route excludes Wi-Fi correction input/output. Wi-Fi continues to serve the tablet and networking diagnostics. Base transmission counts do not prove Rover reception: this first live correction channel is one-way and has no delivery acknowledgement. SiK RSSI is unavailable here; the Rover status API does not substitute Wi-Fi RSSI or Wi-Fi peer age for it.
...
Base generates the session; Rover supplies that returned integer (1000000–4294967295). `{"op":"corrections","transport":"wifi","confirm":true}` selects Wi-Fi. A 202 means queued, not applied; read `corrections` for the result. Latest takeover still wins, without a PIN.
...
After the USB upload is finished, power each complete instrument using its established supplies: ESP32, UM980 and radio must all remain powered, with their common grounds and antennas connected. Disconnect the PC and move the antennas into open sky, keeping Base stationary. A power interruption restores the preview's Wi-Fi correction default, so select SiK only after the outdoor power arrangement is stable.

On the tablet, open Base's `/diagnostics` (use its reachable router address or the address shown by the instrument). Take control and start a new SiK session. Record the displayed session number, then open Rover's `/diagnostics`, take control and join that session. Switching tablet networks between instruments is fine; the SiK data path does not require simultaneous browser connections. In local-router mode the tablet must be able to reach that router; the Rover also exposes the Phone/Tablet access point. If Base's current router address cannot be reached outdoors, arrange access before moving; no persistent/network-independent automatic pairing is implemented yet.
