...
Dashed connections are the selected SiK instrument integration. The ESP32 standalone synthetic diagnostic is implemented, and the production SiK RTCM bridge shipped and passed its first bench validation with real receivers on 2026-09-14: the Base UM980 emitted its full RTCM profile over RTM1 envelopes, the Rover reassembled about 94% of the submitted frames, and every delivered frame was forwarded to its UM980 with zero output faults. Earlier radio PC USB tests found losses/corruption. On 2026-09-12 both ESP32s were connected to radio TTL interfaces, with UM980s disconnected; After correcting reversed TX/RX wiring, both local UART identity probes and a 117-packet-per-direction SiK test passed. This is close-bench evidence only; outdoor `RTK FIXED`, recovery under sky and range remain pending. See the [live correction bridge](live-correction-bridge.md) and the [2026-09-14 SiK bench record](../tests/2026-09-14-sik-bench/README.md). Current saved radio serial speed is 57600 after the comparison test, with no demonstrated cure. The existing COM2/Wi-Fi path previously produced RTK FIXED; that does not qualify the newly reported faulty carrier or establish survey accuracy.
