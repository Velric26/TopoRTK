...
4. Take control on **one** instrument and choose the test there: the tested link and the shape that link offers — duration, framed rate and direction for Wi-Fi, duration and RTCM profile for the SiK test, which is always Base → Rover. Starting it requests a pair-wide operation; both instruments arm the same internal run id and parameters from that request, so nothing is typed twice and there is no session code. Start with 30 seconds, 1000 framed bytes/second and both directions on the bench. One instrument must be Base and the other Rover.
5. Confirm the request. Both peers arm automatically and start after a short countdown; tests last 30, 60, 120 or 300 seconds followed by five seconds to drain packets. Choose 300 seconds when moving apart during a range test. A request refused because the tested medium is unavailable, or because the instrument is busy collecting or updating, is reported as such — a working *other* link can never make a test pass.
