...
## Bench note — a flat pack plus saved `NIGHT` brightness looked exactly like a dead unit (2026-09-15)

Two independent faults stacked, and the operator confirmed the first one: **Unit B's 3S pack was flat**, which is why it vanished from Wi-Fi and why a power cycle changed nothing (a flat pack cannot boot). Once USB powered the board it ran normally — but the panel stayed dark, because the unit's saved brightness mode was `NIGHT`:

```
BRIGHTNESS: NIGHT current=26 target=26 PWM=PASS hardware_duty=26 frequency=5000 Hz   (~10 % backlight, mode stored in device_config)
```

`brightness auto` over the serial console answered `CONFIG SAVE: PASS` and the panel came back at `AUTO NO GPS current=255 target=255 hardware_duty=256`. So a dark screen has two cheap explanations to rule out before suspecting hardware: check the pack, then check the brightness mode over serial (`brightness?`, `brightness auto|day|night`). The green LED beside the USB port is a hardware power/charge indicator; the firmware drives no LED. Nothing on either unit reports its own battery state — that gap is now an open hardware task in [docs/power.md](../../docs/power.md#state-of-charge-monitoring-open--find-the-cheap-simple-way).

A second symptom was misleading: the unit's HTTP timed out at the moment of the report but recovered on its own, while the Wi-Fi counters showed `gap`/`bad` climbing — short timeouts on this bench link are not proof that an instrument is down. The pair also later reported `peer_state=peer_unreachable` with datagrams still arriving and `sequence_gaps`/`invalid_packets` growing, which is loss on this bench link rather than a firmware regression.
