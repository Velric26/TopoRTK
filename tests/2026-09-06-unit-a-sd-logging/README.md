...
- The additional 8 GB card was formatted as `UNIT-B` (FAT32), installed in Unit B, and verified after upload on COM10: `SD: MOUNT PASS card=7494 MB free=7477 MB` and `SD: READBACK PASS`. Unit B completed its UM980 handshake and applied `ROVER SURVEY` automatically.
- PC read-back of the `UNIT-B` card found session `BOOT-2384` with 2,013 one-hertz solution rows spanning 05:30:38–06:04:11 UTC. The rover progressed `GPS FIX` → `DGPS` → `RTK FIXED` at 05:31:11 UTC and remained fixed for 1,980 samples.
- During the RTK-fixed interval, reported H-ACC ranged from 1.5 to 2.4 cm (1.5 cm average), with 15–27 satellites (25 average). RTCM reception reached 8,362 frames and 1,301,176 forwarded bytes; Wi-Fi sequence gaps and invalid packets were both zero. Link RSSI ranged from -68 to -48 dBm (about -60 dBm average).
