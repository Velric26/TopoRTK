...
The existing OTA runner validates the selected unit but still requires explicit URLs. Identity-based discovery across the two known addresses is recommended and documented as a follow-up, not an implemented capability.
...
**2026-09-15 R10d feature request (documented; needs hardware):** The operator asks that the power-on default stay Wi-Fi and that the SiK radios be **off** — or in a low-power mode — until radio communication is requested, so a Wi-Fi-selected instrument does not pay an idle 1 W module's current on the 3S pack. The Wi-Fi default is already the behaviour (a fresh instrument defaults to Wi-Fi and a stored selection is restored); the radio power half is **not implemented and cannot be until the module's supply is switched**. The request is recorded as R10d in the [roadmap](docs/architecture-review.md) with its hardware prerequisite (a GPIO-driven high-side switch, inrush handling, a UART back-power rule for an unpowered module, and a decision between a switched rail and a verified low-power command), the firmware contract for when it exists (energize before a probe/test/radio cutover, keep settling inside the existing windows, never report a powered-off radio as a fault) and its acceptance (measured idle-current comparison plus the R11 rows re-run with the switch in both states). Electrical detail is in the [radio document](docs/radio.md) and the [power document](docs/power.md).
...
**Temporary receiver exception (2026-09-11):** one UM980 carrier has a COM1 transmit fault. A replacement is planned. Recommended interim assignment: known-good receiver as Base, suspect receiver as Rover using COM2 and the existing ESP32 Wi-Fi bridge. No role change has been applied by this note. Direct SiK use of the suspect RXD1 remains untested, and an ESP32-to-SiK serial bridge remains unimplemented. See the [temporary implementation and replacement plan](docs/hardware/unicore-um980/temporary-com1-fault-plan.md). **Correction (2026-09-14):** the ESP32-to-SiK serial bridge is no longer unimplemented — firmware 0.10.4 carries the ESP32 UART2 → RTM1 → COM2 correction bridge and the 2026-09-14 bench session validated it with real receivers (~94% of submitted frames delivered, every delivered frame forwarded into the Rover UM980 with zero output faults; [live correction bridge](docs/live-correction-bridge.md), [bench evidence](tests/2026-09-14-sik-bench/README.md)); direct SiK use of the suspect RXD1 is still untested.
...
| 1 pair | Holybro SiK Telemetry Radio, long-range 1 W, 915 MHz, open source | Base-to-rover RTCM transport | USB configuration backed up and saved/restart-verified; ESP32/RTCM integration, range, and legal use pending; see [radio record](docs/radio.md). **Correction (2026-09-14):** the ESP32-to-SiK RTCM path ran end-to-end on the bench with real receivers (~4.0 RTCM frames/s, ~94% delivered, every delivered frame forwarded into the Rover UM980), so integration is no longer pending; outdoor range, recovery and legal use remain open ([bench evidence](tests/2026-09-14-sik-bench/README.md)). |
...
The selected SiK integration now uses a separate ESP32 UART (TX GPIO17 / RX GPIO18) at each end and keeps UM980 COM2 on GPIO43/44. The owner has no camera and does not plan one, releasing the shared camera GPIO17/18 for this purpose. This avoids the suspect COM1 and supports later status/control traffic; unlike the earlier direct-receiver proposal, ESP32 restart will interrupt corrections. The bridge firmware, stream separation and transport-aware readiness checks remain unimplemented. See [radio wiring and configuration](docs/radio.md). **Correction (2026-09-14):** all three have since landed — the ESP32 UART2 → RTM1 → COM2 bridge in 0.10.4 (bench-validated with real receivers on 2026-09-14), UART2 stream separation under a sole `radio_transport` owner in R3, and transport-aware readiness in R4 ([live correction bridge](docs/live-correction-bridge.md), [bench evidence](tests/2026-09-14-sik-bench/README.md)).
...
**Correction (2026-09-14):** the SiK bench session superseded the “Insert the SiK pair at low power on the bench” and “Compare transmitted and received correction streams” items above — the pair ran at low bench power with real receivers (~4.0 RTCM frames/s, ~94% delivered, every delivered frame forwarded into the Rover UM980; [bench evidence](tests/2026-09-14-sik-bench/README.md)). Outdoor range, recovery and RTK `FIXED` validation remain open, and the USB binary-transfer byte-loss item is still open (see [radio record](docs/radio.md)).
...
- Switched SiK radio supply (R10d): choose and record the GPIO, switch/FET and inrush handling, add the UART back-power rule, and verify on the actual modules whether a dependable low-power command can replace the switched rail.

## 13. Project Documentation

Create focused documents as details become real and testable:

- [PROJECT.md](PROJECT.md) — Project scope and current system decisions
- [docs/hardware/](docs/hardware/README.md) — Hardware index plus one self-contained folder per device
- [docs/firmware.md](docs/firmware.md) — Build, flash, configuration, and firmware architecture
- [docs/gnss-rtcm.md](docs/gnss-rtcm.md) — UM980 configuration and RTCM message set
- [docs/radio.md](docs/radio.md) — SiK connector, configuration, compliance, and range tests
- [docs/power.md](docs/power.md) — Battery, regulator, fuse, BMS, and USB power architecture
- [docs/coordinates.md](docs/coordinates.md) — UTM, datum/epoch, geoid, localization, and base control
- [docs/interface.md](docs/interface.md) — Local and mobile UI behavior
- [docs/validation.md](docs/validation.md) — Test procedures and acceptance criteria
- [docs/decisions.md](docs/decisions.md) — Short architecture decision records
- [tests/](tests/README.md) — Test template and dated test records
...
Retired checkpoint narrative, one row per block removed from the debug log and revision history. Detail and raw evidence live in the linked records; live R/F roadmap items stay in section 2 above.

| Date | Event / version | Outcome | Evidence |
|---|---|---|---|
| 2026-09-10 | UI 0.4 web interface checkpoint | Point review/offline plotting (roadmap feature 6) and the export/backup part of feature 7 passed read-only browser/persistence/export bench checks on hash-verified A and B flashes; CSV import and features 8–12 stayed outstanding, and no field accuracy claim follows. | [implementation state](docs/survey-workflow.md), [evidence](tests/2026-09-10-roadmap-6-12/README.md) |
| 2026-09-11 | UI 0.5/0.6 instrument and Android scope review | The [responsibility review](docs/esp32-android-feature-split.md) deferred bulk project/file processing, continuous-topo session management, localization and full COGO to the phone/app; bounded reference targets, check/repeat occupations, basic point stakeout and manual line tags completed the agreed ESP32 scope with A/B flashes and read-only hardware checks. | [implementation state](docs/survey-workflow.md), [manual-line validation](tests/2026-09-11-manual-lines/README.md) |
| 2026-09-11 | UI 0.7 web GUI redesign | Jobs/Setup/Collect/Points simplified with phone navigation, four expandable setup groups, a compact collection form and separate point tools, sharing the new light field palette; workflow/export, responsive and enlarged-text checks, hash-verified A/B flashes and read-only hardware persistence checks passed, while real Android/field acceptance stayed open. | [interface design](docs/interface.md), [evidence](tests/2026-09-11-gui-redesign/README.md) |
| 2026-09-11 | UI 0.8 Rover control takeover | The Rover web PIN was removed so the latest accepted takeover is the only controller; Base PIN protection still stood at that checkpoint (removed by the next day's follow-up) and both hash-verified flashes passed native/browser and real two-browser takeover checks with no survey records or receiver commands changed. | [validation](tests/2026-09-11-rover-takeover/README.md), [control/Wi-Fi behavior](docs/survey-workflow.md) |
| 2026-09-12 | Standalone link diagnostic checkpoint | Tablet-controlled ESP32 Wi-Fi/SiK diagnostics, latest-report persistence and the electronics/field-test documentation landed; Wi-Fi and (after correcting reversed TX/RX) SiK each passed 117/117 synthetic packets both directions at 1000 framed bytes/second for 30 seconds, with the UM980s disconnected and the SiK antennas about 10 cm apart, so no range or survey qualification follows. | [evidence](tests/2026-09-12-transport-test-kit/README.md) |
| 2026-09-12 | Tablet workflow and takeover follow-up | Both roles moved to latest-request takeover with the old Base PIN path and touchscreen prompts removed, and the diagnostic UI gained live/stale connection state, reload and current-versus-saved report download. SiK delivery stayed unqualified: the 30 cm two-minute run received 467/468 at Base and 465/468 at Rover, and the six-metre repeat (run 913205) received 465/468 at both ends. | [evidence](tests/2026-09-12-standalone-tablet/README.md), [six-metre run](tests/2026-09-12-standalone-tablet/sik-913205/README.md) |
| 2026-09-12 | Loss-tolerant transport plan | Plan of record: separate fresh RTCM delivery, acknowledged duplicate-safe commands and periodic status; build the fixed-memory core and on-instrument fault self-tests first, then paired synthetic SiK, then real UM980 COM2/RTK validation, keeping both UM980s disconnected through the synthetic stages. | [loss-tolerant transport](docs/loss-tolerant-transport.md) |
| 2026-09-12 | Transport Stage 1 (local fault checks) | Stage 1 implemented and flashed to both ESP32s: all 22 local transport fault checks pass, saved reports survive restart, and the fixed-memory core's full ESP32 test workspace is 4744 bytes, driven from `/diagnostics` → **Run local fault checks**. | [evidence](tests/2026-09-12-correction-transport/README.md) |
| 2026-09-13 | 0.10.5 source (Debug/OTA preparation; not flashed) | Touchscreen Debug, a 15-minute user-idle timeout, a disabled-with-instructions web tab and bounded passive communications capture built with host/browser checks passing, but the increment was never flashed or hardware accepted, so the deployed baseline stayed 0.10.4 and upload stayed disabled. | [Debug/OTA plan](docs/debug-and-ota.md), [validation record](tests/2026-09-13-debug/README.md) |
| 2026-09-13 | Update-notice core | The portable paired update-notice codec and state machines passed corruption, replay/reorder, bounded retry/deadline and recovery tests; transport/UI integration and the actual OTA stayed outstanding. | [notice core evidence](tests/2026-09-13-update-notice/README.md) |
| 2026-09-14 | 0.11.0 Debug OTA workflow | Debug OTA and paired update notices landed with both units USB-installed and verified; the first Unit B Wi-Fi upload timed out after 37,336 bytes with the old boot still running and update lock/pause cleared, so successful OTA and rollback acceptance stayed pending. | [live evidence](tests/2026-09-14-ota-live/README.md), [USB evidence](tests/2026-09-14-ota-usb/README.md), [OTA operator guide](docs/ota-operator-guide.md) |
| 2026-09-14 | 0.11.1 upload recovery | Bounded receive-timeout retries, prompt closure of failed uploads and paused in-flight browser Debug requests; Unit B was USB-installed and verified at 0.11.1 with saved survey state preserved. | [validation and remaining limits](tests/2026-09-14-ota-recovery/README.md) |
| 2026-09-14 | 0.11.2 first successful OTA | Unit B updated over local Wi-Fi to 0.11.2 in 118.602 s with no receive-timeout retries, rebooted and passed startup acceptance with saved state unchanged; the release fixes a reproduced stale-clock race, and the transfer left the 120-second deadline nearly exhausted. | [hardware evidence](tests/2026-09-14-ota-success/README.md) |
| 2026-09-14 | 0.11.3 timeout changes | The OTA transfer deadline rose from 120 to 300 seconds and Debug stopped expiring after 15 idle minutes (staying On until disabled or restarted, with the `activity` keep-alive operation and browser extend control removed); both units took 0.11.3 over Wi-Fi OTA with acknowledged paired notices, verified boots and unchanged saved survey state. | [test record](tests/2026-09-14-timeout-changes/README.md) |
| 2026-09-14 | 0.11.4 merged link card | The Correction Link and Link Signal cards merged into one **BASE/ROVER LINK** indicator (transport first, then `Disconnected` or the signal strength in dBm, `Radio Connected` for a linked SiK) and the dashboard reflowed to three equal cards; both units took 0.11.4 over Wi-Fi OTA inside the new 300-second deadline. | [test record](tests/2026-09-14-merged-link-card/README.md) |
| 2026-09-14 | 0.11.5 Debug default-on | Debug became On by default at startup, with a manual disable persisting only until the next restart; both units took 0.11.5 over Wi-Fi OTA and the post-reboot Debug state was asserted On, which removed the touchscreen enable step before updates. | [test record](tests/2026-09-14-debug-default-on/README.md) |
| 2026-09-14 | Rollback acceptance (P0 complete) | Flag-guarded fault-injection images proved the OTA safety paths on hardware: a pending boot forced to fail the health gate was marked invalid and rolled back by the bootloader (~30 s), a hung pending boot was recovered by the task watchdog, and a raw-socket mid-upload disconnect was caught by the receive-error path and the 12-second stall detector, with firmware, boot and records retained. | [rollback acceptance evidence](tests/2026-09-14-rollback-acceptance/README.md) |
| 2026-09-14 | SiK bench session (P0 complete) | The production SiK correction link ran end-to-end with real receivers for the first time: both radios probed as `RFD SiK 2.0 on HM-TRP` on UART2, the paired synthetic radio test passed 15/15 both directions, and a live session carried ~4.0 RTCM frames/s at ~94% delivered with every delivered frame forwarded into the Rover UM980 and zero output faults; indoor bench range and outdoor RTK `FIXED`/recovery validation stayed open. | [bench evidence](tests/2026-09-14-sik-bench/README.md) |
| 2026-09-14 | Repository restructure (housekeeping) | `firmware/um980-display-demo/` — the production firmware that grew out of the first display demo — was flattened to `firmware/`, and the early bring-up prototype `firmware/waveshare-board-demo/` was archived to `archive/waveshare-board-demo/`. Living documentation, tooling paths and script-relative depths were updated, and dated records under `tests/` intentionally keep the historical paths. Host tests, both PlatformIO builds and packaging verified from the new root with no firmware behavior change. The git-ignored `firmware/.pio/` moved intact: it holds the private `ota-live-*` OTA evidence directories referenced by the [R2b deployment record](tests/2026-09-14-ui-r2b-deployment/README.md) and the local credentials. | [post-restructure record](tests/2026-09-14-arch-r4-status/README.md) |
