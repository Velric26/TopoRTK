...
**2026-09-14 update:** 0.11.0 implements the Debug OTA workflow and paired update notices. Build/host/browser checks pass. Unit A/Base is USB-installed and verified with saved survey state preserved and Debug Off; Unit B/Rover remains on 0.10.4. Further flashing is paused at the user's request. Actual Wi-Fi OTA and rollback acceptance remain pending. Full flash backups are optional for routine development; prefer a small settings snapshot when practical. SiK correction recovery requires a fresh Base session and Rover rejoin after an update. See the [operator guide](docs/ota-operator-guide.md) and [USB evidence](tests/2026-09-14-ota-usb/README.md). Earlier entries below describe historical checkpoints.
