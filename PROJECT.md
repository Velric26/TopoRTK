...
- Repository restructure (accepted, deferred): archive `firmware/waveshare-board-demo/` to `archive/` and flatten `firmware/um980-display-demo/` contents up into `firmware/` so the production firmware no longer carries the stale demo name. Execute only after the paused R3 tree is parked or reverted; the full step list, `.gitignore` hazard and verification gates are in the [restructure plan](docs/firmware-folder-restructure.md).
